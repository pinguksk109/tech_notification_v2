...
impl ArticlePort for ZennArticleRepository {
    async fn fetch_items(&self, page: usize) -> Result<Vec<Item>> {
        ...
        let resp = self
            ...
            .send()
            .await
            .with_context(|| format!("Zenn request failed: {}", url))?;

        if resp.status() != StatusCode::OK {
            anyhow::bail!("Zenn API error {}", resp.status().as_u16());
        }

        let body: Value = resp.json().await.context("Zenn: parse JSON")?;
        ...
        let items: Vec<Item> = raw_articles
            ...
            .map(|art| {
                ...
                let likes_count =
                    art.get("liked_count").and_then(Value::as_u64).unwrap_or(0) as u32;
                Item {
                    title,
